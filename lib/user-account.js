...
//
// The `users` collection has a unique index on `email` (from the frontend's Mongoose model).
// A brand-new document created here rarely has a real email available, and leaving the field
// missing/null is NOT safe — MongoDB's unique index treats every doc missing the field as
// sharing the same null value, so only the very first such user can ever be created; every
// subsequent one fails with a "DuplicateKey" 500 on email: null. Always fall back to a
// synthetic-but-guaranteed-unique email (derived from clerkId) when no real one is supplied.
  ...
  if (existing) {
    ...
    const update = {
      $set: {
        ...
        // Backfill a missing email on an already-broken record (created before this fix) so it
        // stops colliding with other users on the next write.
