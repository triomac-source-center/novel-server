  ...
  try {
    ...
    await session.withTransaction(async () => {
      await Deposit.create(
        [{ userId: record.userId, txid: transfer.transaction_id, amount, address: record.address, status: "confirmed" }],
        { session }
      );
...
export async function runDepositDetection() {
  ...
  return { checked: addresses.length, credited, alreadyProcessed, errors };
}
